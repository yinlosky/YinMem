function saveVectorToEachMachine()
%%%%
...
%% TO save space, my process should be good enough to identify its own machine so that each local machine has a vector copy in its own Alluxio underFSStorage. Technically stored in each process.
...
%% 
%%
%%	Note: 1. eval(pRUN('saveVectorToEachMachine'), NumOfProcessors, machines); 
...
for i = myMachine
	...
	outputFilePath = [outputFilePathPre '/' num2str(it) 'v_' num2str(NumOfNodes) 'nodes_' num2str(NumOfProcessors) 'proc_' num2str(i) '_id' ];
