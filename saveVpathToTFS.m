function saveVpathToTFS()
%%%%
%%  usage: This function serves to save the calculated {NumOfNodes}lz_vpath_global(-r -v) to local machine in the cluster named with {NumOfNodes}lz__vpath_{machineName}_{r,v}
...
%% 
%%
%%	Note: 1. eval(pRUN('saveVpathToTFS'), NumOfMachines, machines); 
...
DebugPath = ([DebugPathPre '/local_lzVpath_' num2str(NumOfNodes) 'nodes']);
if(~exist([DebugPath] , 'dir'))
        mkdir([DebugPath] );
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% input file path is [num2str(NumOfNodes) 'lz_vpath_global' {_r _v}]
%%%%%%%input_path_t = DB([num2str(NumOfNodes) 'lz_q' num2str(it)]);
% /mytest/1048576lz_vpath_global_r ,v
inputFilePathPre = '/mytest';
inputFilePath=[inputFilePathPre '/' num2str(NumOfNodes) 'lz_vpath_global' ];
...
for i = myMachine
	...
	outputFilePath = [outputFilePathPre '/' num2str(NumOfNodes) 'lz_vpath_' mymachine];
